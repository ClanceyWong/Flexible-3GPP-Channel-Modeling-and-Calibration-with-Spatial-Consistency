...
N0_dBm      = -174;              % noise spectral density
...
% today = '2020-08-19';
if ~exist(['.\results\TR38900\full_config1_result','_',today],'file') 
    mkdir(['.\results\TR38900\full_config1_result','_',today]);
end 
...
%         plot(sort(sf), result.Pr, 'linewidth',1.5);grid on;hold on;
        ...
        fname = ['.\results\TR38900\full_config1_result','_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
