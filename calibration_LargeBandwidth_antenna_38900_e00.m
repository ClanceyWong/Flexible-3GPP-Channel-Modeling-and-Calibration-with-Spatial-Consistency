...
% today = '2020-08-06';
if ~exist(['.\results\TR38900\LargeBandwidth_result','_',today],'file') 
    mkdir(['.\results\TR38900\LargeBandwidth_result','_',today]);
end 
...
for frequency = [3e10]
    for case_num = 2
        ...
        BSant_params.array.Ng           = 4;
        BSant_params.array.dg_H         = 4;
        BSant_params.array.dg_V         = 4;
        BSant_params.panel.M            = 8;                 %  the number of antenna elements with the same polarization in each column.
        BSant_params.panel.N            = 8;
        BSant_params.panel.Kv           = 8;                  %  K = M or 1
        BSant_params.panel.Kh           = 8;
...
%         SA4          = zeros(4,numel(UE_list));
        ...
        for nn = 1:numel(UE_list)
            ...
            for n2 = 1:numel(BS)
                ...
                for n3 = 1:sec_per_BS
                    [RSRPt(3*(n2-1)+n3), couplingloss(3*(n2-1)+n3)] = Ue.RSRP_calc_LBW(BSsector_list(site.sector(n3)),link,port0);
                end
...
%             CouplingLoss(n1) = link.CouplingLoss; 
            [~,~,~, SV] = Ue.get_channel_LBW(BSsector_list(link_id),0);
            LSV(n1)     = max(SV);
...
%         plot(sort(sf), result.Pr, 'linewidth',1.5);grid on;hold on;
        ...
        figure(4);
        plot(result.LSV, result.Pr, color, 'linewidth',1.5);grid on;hold on;
        title('CDF of Delay spread');xlabel('DS (ns)');ylabel('CDF'); xlim([-30, 40])
...
%         title('CDF of ASA');xlabel('ASA (degree)');ylabel('CDF');xlim([0, 120])
        ...
        fname = ['.\results\TR38900\LargeBandwidth_result','_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
