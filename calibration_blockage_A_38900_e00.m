...
% today = '2020-08-06';
if ~exist(['.\results\TR38900\blockage_result_A_',today],'file') 
    mkdir(['.\results\TR38900\blockage_result_A_',today]);
end 
...
for frequency = [3e10]
    for case_num = 2
        ...
        % drop BS
        [BS, BSsector_list]    = network_layout.Drop_BaseStation(scenario, BSant_params, sec_per_BS, frequency, version, plot_flag, 'blockage'); % 
        % drop UE
...
%             DS(n1)       = link.calc_delay_spread(link.tau_n, link.Pn );
            ASA(n1)    = link.calc_angular_spreads(link.phi_n_m_AOA,link.Pn,link.phi_LOS_AOA);
...
%         plot(sort(sf), result.Pr, 'linewidth',1.5);grid on;hold on;
        ...
        figure(4);
        plot(result.ASA, result.Pr, color, 'linewidth',1.5);grid on; hold on;
        title('CDF of ASA');xlabel('ASA (degree)');ylabel('CDF');xlim([0, 120])

        fname = ['.\results\TR38900\blockage_result','_A_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
