function [ objsos, ase, test_dir ] = generate_sos( R, D, N, dim, uniform_smp, T, random_init, show_progress )
% GENERATE_SOS Generates SOS parameters from a given ACF
%
% This function tries to approximate any given ACF by sinusoid coefficients. A sampled ACF is provided at the input.
% Then, the N sinusoid coefficients are iteratively determined until the best match is achieved. 
%
...
%   uniform_smp     If set to 1, sample points for 2 or more dimensions are spaced equally. If set to 0, sample points are chosen randomly. Default: 1 (uniform)
%   T               Number of test-directions for N-dimensional approximation
%   random_init     If set to 1, the SOS frequencies are randomly initialized. Default: 0 (use initial search)
...
switch dim
    ...
    case 2
        ...
        else % Random sampling of the unit circle
            phi = 2*(rand(N,1)-0.5)*pi;
        end
        u        = 2 * pi / T;                  % Circle circumference
        phi_t    = 0 : u : 2*pi;               
        phi_t    = angle(exp(1j*phi_t(1:T))).'; % Azimuth test direction
        theta_t  = [];                          % Elevation test direction
        test_dir = phi_t;
        
    case 3
        if uniform_smp
            [ theta, phi ] = tools.SpherePack( N );
        else % Random sampling of the unit sphere
...
% Transform input tensor into a matrix for easier processing Single precision provides 
% sufficient numeric accuracy but reduces computing and memory requirements.
R = single(R(:).');
D = single(D(:).');

% Generate SOS object
objsos      = SOS.sos([]);
objsos.dist = [D(1),D(2),D(end)];
objsos.acf  = R;
...
if random_init % Random values for the initial root frequencies
...
while cnt > 0
    ...
    for n = 1 : N
        ...
        f = SOS.iteration_search( RminusRr, Dnorm, an );
...
% Save output
objsos.sos_freq = fr;
objsos.sos_amp = sqrt( single( 2 /  N ));
objsos.init;
