...
%
% Input (constructor):
%   acf_type:   Default: 'Comb300'
%   String describing the shape of the autocorrelation function and the number of sinusoids
%
%   distribution:   Default: 'normal'
%   Distribution of the random variables ('normal' or 'uniform')
%
%   dist_decorr:   Default: 10 m
%   Decorrelation distance in [m]

properties
    name         = 'Custom';    % Name of the preset
    distribution = 'Normal';    % Distribution of random variables ('Normal' or 'Uniform')
    
    dist         = [];          % Vector of sample points for the ACF in [m]
    acf          = [];          % Desired aotocorrelation function
...
properties(Dependent)
    ...
    no_coefficients             % Number of sampling frequencies
end

methods
    % Constructor
    function obj = sos( acf_type, varargin )
        if exist('acf_type','var') && ~isempty( acf_type ) && nargin > 0
            obj = obj.set_acf( acf_type , varargin{:} );
        elseif nargin == 0
            obj = obj.set_acf( 'Comb300' );
        end
    end
    
    % Get functions
    function out = get.dist_decorr(obj)
        [~, indx] = min( abs(obj.acf - single(exp(-1))));
        d         = obj.dist(1):obj.dist(2):obj.dist(3);
        out       = single( d( indx ) );
    end
    function out = get.dimensions(obj)
        out = size( obj.sos_freq, 2 );
    end
    function out = get.no_coefficients(obj)
        out = size( obj.sos_freq, 1 );
    end
    
    % Set functions
    function set.dist_decorr(obj,value)
        if ~( ( all(size(value) == [1 1]) || all(size(value) == [1 2]) ) && isnumeric(value) && isreal(value) && all(value > 0) )
            error('sos:wrongInputValue','"decorr_dist" must be scalar and positive.')
        end
        if isempty( obj.dist )
            error('sos:noACFgiven','No ACF specified')
        end
        ...
        maxD_old = obj.dist(end);
        obj.dist = value(1) ./ obj.dist_decorr(1) .* obj.dist;
        
        % Adjust the frequencies
        maxD_new = max(obj.dist);
        obj.sos_freq = obj.sos_freq * maxD_old / maxD_new;
        
        % Save new decorr distance
        obj.dist_decorr = value;
    end
    
    function set.distribution(obj,value)
        ...
        if ~( ischar(value) && any( strcmpi(value,supported_types)) )
            ...
            for n = 1:no
                str = strcat(str,supported_types{n});
                if n<no
                    str = strcat(str,',  ');
                end
            end
            error('sos:wrongDistributionType',str);
        end
        obj.distribution = value;
    end
    
    %%
    function obj = set_acf(obj, acf_type, distribution, dist_decorr )
        % Description:
        %        The 3D SOS frequencies are precomputed for 150, 300, 500, and 1000 sinusoids.
        ...
        switch acf_type
            ...
            case {'Comb150','Comb300','Comb500','Comb1000'}
                R = exp(-d.^2./(10.^2)).*(d<=10) + exp(-d./10).*(d>10);
            otherwise
                error('sos:type','ACF type not found.');
        end
        ...
        obj.dist     = single( D );
        obj.acf      = single( R );
        obj.sos_freq = single( F ./ D(end) );
        obj.sos_amp  = sqrt( single( 2 /  size(F,1)) );
        obj.init;
        ...
        if exist( 'dist_decorr','var' ) && ~isempty( dist_decorr ) && any( dist_decorr ~= 10 )
            obj.dist_decorr = dist_decorr;
        end
        
    end

    %%
    function init( obj, use_same )
        % Initializes the random phases
        % Input:   use_same
        %   If set to 1 (default), identical phases are used for the dual-mobility option, assuming that devices are on the same radio-map. If set to 0, different values are used.
        if ~exist( 'use_same','var' ) 
            use_same = true;
        end
        if use_same
            obj.sos_phase = single( 2*pi*(rand(obj(1,1).no_coefficients,1)-0.5) ) * single([1,1]);
        else
            obj.sos_phase = single( 2*pi*(rand(obj(1,1).no_coefficients,2)-0.5) );
        end
    end
    
    %%
    function s = value( obj, ca, cb )
        ...
        elseif any( size(cb) ~= size(ca) )
            error('sos:value','Size of "cb" must match the size of "ca".' );
        end
        ...
        al   = obj.sos_amp;
        ph   = obj.sos_phase;
        fr   = 2*pi*obj.sos_freq;
        L    = size(fr,1);
        dims = size(fr,2);

        if size( ca, 1 ) > dims
            error('sos:value','Number of requested dimensions is not supported.' );
        elseif size( ca, 1 ) < dims
            ca = [ ca; zeros( dims - size(ca,1), no_value,'single' ) ];
        end
        if ~isempty(cb) && size( cb, 1 ) < dims
            cb = [ cb; zeros( dims - size(cb,1), no_value,'single' ) ];
        end

        % Calculate the random variables for the first input coordinate
        if no_value < 1e4
            sa = al .* sum( cos( mod( fr * ca + ph(:,ones(1,no_value,'uint8')) , 2*pi )),1);
        else
            sa = zeros( 1,no_value,'single' );
            for l = 1:L
        ...
        if ~isempty( cb )
            ...
            if numel( obj.dist_decorr ) == 2
                fr = fr .* ( obj.dist_decorr(1) / obj.dist_decorr(2) );
            end

            % Calculate the random variables for the second input coordinate
            if no_value < 1e4
                sb = al .* sum( cos( mod( fr * cb + ph(:,ones(1,no_value,'uint8')*2) , 2*pi )),1);
            else
                sb = zeros( 1,no_value,'single' );
                for l = 1:L
            ...
            if all( abs( ph(:,1) - ph(:,2) ) < 1e-6 )
                ...
                % Interpolate the ACF
                D( D>obj.dist(end) ) = obj.dist(end);
                A = tools.interp( obj.dist, 0, obj.acf, D );
    ...
    function out = copy( obj )
        ...
        prop = {'name','distribution','dist','acf', 'sos_freq','sos_phase','sos_amp'};
        out = SOS.sos([]);

        % Copy the data
        for n = 1 : numel(prop)
            out.( prop{n} ) = obj.( prop{n} );
        end
    end
    
    
    
    function F = get_frequency(obj)
        switch obj.name
            case 'Exp150'
    ...
    function [ Ro, Do ] = acf_approx( obj )
        %  Generates the approximated ACF from SOS coefficients
        %
        %   This method calculates the approximated ACF. The distance ranges from -2 Dmax to 2 Dmax, where Dmax is the maximum
        %   value in sos.dist. The format of the output variable (Ro) depends on the number of dimensions.
        %
        %   - 1 dimension :   Ro is a vector contining the values at the sample pints of Do.
        %   - 2 dimensions:   Ro is a matrix containing the approximated 2D ACF
        %   - 3 dimensions:   Ro contains 3 matrices, one for the x-y plane, one for the x-z plane and one for the y-z plane.
        %
        % Output:
        %   Ro      Approximated ACF
    ...
    function [ Ri, Di ] = acf_2d( obj )
    %ACF_2D Interpolates the ACF to a 2D version
    %
    %   This method calculates a 2D version of the given ACF (sos.acf). The distance ranges from -2 Dmax to 2 Dmax, where
    ...
    %   Di      Vector of sample points (in x and y direction) for the 2D ACF in [m]
        ...
        Ri(ind) = tools.interp( D, 0, R, Dc(ind) );
    end
    
    function save( obj, filename ) 
        % Saves the coefficients to a mat-file
        % Input:
        %   filename 	Path or filename to the coefficient file. 
        if ~exist( 'filename', 'var' )
           error('Please give the path or filename');
        end
        sos_obj = obj;
        save( filename,'sos_obj' );
    end
end
