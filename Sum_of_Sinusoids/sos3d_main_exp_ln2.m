...
d_cor  = 10;                  % 自相关距�?
ACFexpln2 = @(d)exp((-log(2)*(d)/d_cor)); % 自相关函数ACF
...
xyz(3,:,:) = dz';
xyz = reshape(xyz,3,[])';
acf = repmat(ACFexpln2(dist_train),1,directions_train)';
...
acf_v = repmat(ACFexpln2(dist_valid),1,direction_valid)';
...
for time = 1:times
    ...
    for N = num_sinusoids
        ...
        xyzt(3,:,:) = dzt';
        xyzt = reshape(xyzt,3,[])';
        acft = repmat(ACFexpln2(dist_test),1,direction_test)';
...
error_3d_last_exp_ln2 = error4plot(times, :);
save error_3d_last_exp_ln2 error_3d_last_exp_ln2;
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % figure
% % scatter3(storedNN.W{1}(:,1),storedNN.W{1}(:,2),storedNN.W{1}(:,3))
% figure(8);hold off;
% plot(dist_train(1:4:end),ACFexpln2(dist_train(1:4:end)),'Linewidth',2 );hold on;grid on;
...
% figure(11);plot(0:70,ACFexpln2(0:70),'Linewidth',1.5);hold on ;
