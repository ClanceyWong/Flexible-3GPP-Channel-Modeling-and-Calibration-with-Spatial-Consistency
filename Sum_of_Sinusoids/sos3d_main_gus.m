...
for time = 1:times
    ...
    for N = num_sinusoids
        ...
        xyzt(3,:,:) = dzt';
        xyzt = reshape(xyzt,3,[])';
        acft = repmat(ACFgus(dist_test),1,direction_test)';
...
error_3d_last_gus = error4plot(times, :);
save error_3d_last_gus error_3d_last_gus;
...
% figure(8);hold off;
% plot(dist_train(1:4:end),ACFgus(dist_train(1:4:end)),'Linewidth',2 );hold on;grid on;
...
% figure(11);plot(0:70,ACFgus(0:70),'Linewidth',1.5);hold on ;
