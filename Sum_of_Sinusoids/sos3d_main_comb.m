...
d_cor  = 10;                  % 自相关距离
ACFcomb = @(d)exp(-(d)/d_cor).*(d>=d_cor) + exp(-(d).^2/d_cor^2).*(d<d_cor); % 自相关函数ACF

% train
prop = (1-exp(-50/d_cor))/200;
k = 0:200;                         % 训练集不同距离的数量
dist_train = -d_cor*log(1-prop*k); % 训练集的距离
directions_train = 56;             % 训练集的每个距离的方向数量
...
xyz(3,:,:) = dz';
xyz = reshape(xyz,3,[])';
acf = repmat(ACFcomb(dist_train),1,directions_train)';
...
acf_v = repmat(ACFcomb(dist_valid),1,direction_valid)';

times = 1; % 重复次数
% num_sinusoids = (100:100:1000); % 正弦波数量
num_sinusoids = 500; % 正弦波数量
...
for time = 1:times
    ...
    for N = num_sinusoids
        ...
        xyzt(3,:,:) = dzt';
        xyzt = reshape(xyzt,3,[])';
        acft = repmat(ACFcomb(dist_test),1,direction_test)';
...
error_log_3d_comb = error_log;
% save error_log_3d_comb error_log_3d_comb;
%% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% % figure
% % scatter3(storedNN.W{1}(:,1),storedNN.W{1}(:,2),storedNN.W{1}(:,3))
% figure(8);hold off;
% plot(dist(1:4:end),R2(dist(1:4:end)),'Linewidth',2 );hold on;grid on;
% rrr = reshape(storedSOS_model.a{3}',numel(dist),[])';
% meanR = mean(rrr); maxR = max(rrr)-mean(rrr); minR = min(rrr)-mean(rrr);
% plot(dist,meanR,'r-','Linewidth',2);
% errorbar(dist(1:8:end),meanR(1:8:end),maxR(1:8:end),minR(1:8:end),'.r-');
...
% qxyz(3,:,:) = z;
% qxyz = reshape(qxyz,3,[])';
% dist = sqrt(sum(qxyz.^2,2));
% qacf          = R2(dist)';
% acf_d  = reshape(qacf,size(x,1),size(x,2));
% [error,storedSOS] = sos_test(storedSOS,qxyz,qacf);
...
% % mesh(x,y,z);
% 
% %% Figure 2. Example of a spatially correlated 3D process
clear pxyz
xy      = 0:100;
zz      = 0:100;
[x,y,z] = meshgrid(xy,xy,zz);
...
% figure(11);plot(0:70,R2(0:70),'Linewidth',1.5);hold on ;
