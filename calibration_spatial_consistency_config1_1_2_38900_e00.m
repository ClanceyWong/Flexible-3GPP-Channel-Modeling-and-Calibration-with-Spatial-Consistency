...
layer_num   = 2;              % the number of layer of cells.
...
% today = '2020-08-06';
if ~exist(['.\results\TR38900\spatial_consistency_result','_config1_',today],'file') 
    mkdir(['.\results\TR38900\spatial_consistency_result','_config1_',today]);
end 
...
%         title('CDF of ASA');xlabel('ASA (degree)');ylabel('CDF');xlim([0, 120])
        ...
        fname = ['.\results\TR38900\spatial_consistency_result','_config1_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
