...
today = '2020-11-09';
if ~exist(['.\results\TR38900\full_config2_result','_',today],'file') 
    mkdir(['.\results\TR38900\full_config2_result','_',today]);
end 
    ...
    for case_num = 3
        ...
        for nn = 1:numel(UE_list)
            ...
            for n2 = 1:numel(BS)
                ...
                for n3 = 1:sec_per_BS
                    [RSRPt(sec_per_BS*(n2-1)+n3), couplingloss(sec_per_BS*(n2-1)+n3)] = Ue.RSRP_calc(BSsector_list(site.sector(n3)),link,port0);
                end
                link_list = [link_list;link]; %#ok<AGROW>
            end
            [pl, link_id]    = max(RSRPt);
            link             = link_list(ceil(link_id/sec_per_BS));
...
%%
        ...
        fname = ['.\results\TR38900\full_config2_result','_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
