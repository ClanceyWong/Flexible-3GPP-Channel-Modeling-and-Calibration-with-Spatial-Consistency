...
% today = '2020-08-06';
if ~exist(['.\results\TR38901\oxygen_absorption_result','_',today],'file') 
    mkdir(['.\results\TR38901\oxygen_absorption_result','_',today]);
end 
...
%         SA4          = zeros(4,numel(UE_list));
        ...
        for nn = 1:numel(UE_list)
            ...
            for n2 = 1:numel(BS)
                site      = BS(n2);
                link      = links.Link38901(site, Ue, scenario, true, tab, 'oxygen absorption');
                for n3 = 1:sec_per_BS
...
%         title('CDF of ASA');xlabel('ASA (degree)');ylabel('CDF');xlim([0, 120])
        ...
        fname = ['.\results\TR38901\oxygen_absorption_result','_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
