...
ue_per_sec  = 500;             % the number of ue per sector.
...
% today = '2020-08-06';
if ~exist(['.\results\TR38901\spatial_consistency_result','_config2_',today],'file') 
    mkdir(['.\results\TR38901\spatial_consistency_result','_config2_',today]);
end 
...
for frequency = [3e10]
    for case_num = 2
        ...
        if strcmp(scenario.name,'3D-RMa') ||strcmp(scenario.name,'3D-UMa')||strcmp(scenario.name,'3D-UMi')
            onlycentre = false;
            [UE_list, ue_pos_list] = network_layout.Drop_UE_spatialConsistency(BSsector_list, UEant_params, ue_per_sec, scenario.R, scenario.min_d, false, onlycentre, beta, plot_flag);
...
%%
        ...
        clear result;
        result.CouplingLoss = sort([UE_list_c.couplingloss]);
        result.SINR         = sort([UE_list_c.SINR_geometry]);
        result.Pr           = (1:numel(result.CouplingLoss))/numel(result.CouplingLoss)*100;
...
%         title('CDF of ASA');xlabel('ASA (degree)');ylabel('CDF');xlim([0, 120])
        ...
        fname = ['.\results\TR38901\spatial_consistency_result','_config2_',today,'\',scenario.name,'_',num2str(frequency/1e9),'GHz.mat'];
        save(fname,'result');
    end
end
