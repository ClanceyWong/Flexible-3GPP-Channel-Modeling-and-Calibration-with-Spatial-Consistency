...
figure(f(3));
h3 = plot(result.LSV, result.Pr,'-.','Marker','s','MarkerIndices',(start:step:numel(result.Pr)),'MarkerSize',12, 'linewidth',2,'color',color(2,:));grid on;hold on;
%         title('CDF of DS');
...
xlabel('Geometry SINR (dB)');ylabel('CDF (%)'); 
figure(f(3));
h3 = plot(result.LSV, result.Pr,'--','Marker','x','MarkerIndices',(start:step:numel(result.Pr)),'MarkerSize',12, 'linewidth',2,'color',color(3,:));grid on;hold on;
%         title('CDF of DS');
xlabel('10log10(Largest singular value)');ylabel('CDF (%)');
hh1 = [h1,h2,h3];
%%
