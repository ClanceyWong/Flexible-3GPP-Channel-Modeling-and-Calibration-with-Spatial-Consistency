classdef Link36873 < handle
    ...
    methods
        function obj = Link(BSsector, UE, scenario, fastfading_enable, tab)
            obj.sector = BSsector;
            obj.UE = UE;
...
%             end
           ...
           %% step 2
            ...
            if UE.rand_los(BSsector.ID) < PrLOS
                obj.bLOS = true;
            else
                obj.bLOS = false;
            end
           ...
           %% step 4
            ...
            if fastfading_enable
              ...
              %% step 9: Generate XPRs
                X_n_m = obj.sigma_XPR*randn(obj.N_new, obj.M)+obj.mu_XPR;
%                 X_n_m(X_n_m<0) = 0;
        ...
        function Pr_LOS = los_probability(obj)
            ...
            elseif strcmp(obj.scenario.name,'InF')
                ...
                switch obj.scenario.InF_case
                    ...
                    case {'SH','DH'}
                        k_subsce = -d_clutter/log(1-r)*((obj.sector.attachedBS.h_BS-h_UT)/(obj.scenario.hc - h_UT));
                    case 'HH'
                        k_subsce = inf;
                    otherwise
                        error(['Case "' obj.scenario.InF_case '" for InF is not valid! ']);
                end
                Pr_LOS = exp(-obj.d_2D_in/k_subsce);
            end
        end
...
% common function
        ...
        function [x,y,z] = sph2cart_(phi, theta, varargin) %#ok<DEFNU>
            if isempty(varargin)
                r = 1;
            else 
                r = varargin{1};
