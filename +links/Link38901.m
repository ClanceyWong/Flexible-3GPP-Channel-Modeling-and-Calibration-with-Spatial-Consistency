classdef Link38901 < handle
    ...
    methods
        function obj = Link38901(BSsector, UE, scenario, fastfading_enable, tab, varargin)
            obj.sector = BSsector;
            obj.UE = UE;
...
%                 obj.tau_n = sort(tau_n_tmp - min(tau_n_tmp));
        ...
        function Pr_LOS = los_probability(obj)
            ...
            elseif strcmp(obj.scenario.name,'InF')
                ...
                switch obj.scenario.InF_case
                    ...
                    case {'SH','DH'}
                        k_subsce = -obj.scenario.d_cluster/log(1-obj.scenario.r)*((obj.sector.h_BS-h_UT)/(obj.scenario.hc - h_UT));
                    case 'HH'
                        k_subsce = inf;
                    otherwise
                        error(['Case "' obj.scenario.InF_case '" for InF is not valid! ']);
                end
...
%                     hE = randsrc(1,1,[12,15,18,21]);
                ...
                if (obj.d_2D >= 10) && (obj.d_2D <= d_BP_tmp)
                    PL_LOS = 22*log10(obj.d_3D)+28+20*log10(fcG);
                elseif (obj.d_2D > d_BP_tmp) && (obj.d_2D < 5000)
                    PL_LOS = 40*log10(obj.d_3D)+28+20*log10(fcG)-9*log10(d_BP_tmp.^2+(h_BS-h_UT).^2);
                end
                if obj.bLOS
                    PL_db = PL_LOS;
                    sigma_SF_db = 4;
                ...
                if ~obj.bLOS
                    switch obj.scenario.InF_case
                        case 'SL'
                            ...
                            sigma_SF_db = 5.7;
                        case 'DL'
%                             PL_dbsl = max(PL_db,33+25.5*log10(obj.d_3D)+20*log10(fcG));
%                             PL_db = max(PL_db,PL_dbsl);
...
%                             sigma_SF_db = 5.9;
                            PL_db = max(PL_db,50.5+12.8*log10(obj.d_3D)+20*log10(fcG));
                            sigma_SF_db = 5.4;
                        case 'DH'
%                             PL_db = max(PL_db,33.63+21.9*log10(obj.d_3D)+20*log10(fcG));
%                             sigma_SF_db = 4;
                            PL_db = max(PL_db,28.6+25.5*log10(obj.d_3D)+20*log10(fcG));
                            sigma_SF_db = 3.6;
                    end
                end
            end          
...
%                     weight_bs = 1; weight_ue = 0;
            ...
            elseif strcmp(obj.scenario.name,'3D-UMi')
                ...
                if obj.O2I % 
                    if obj.bLOS
                        ...
                        obj.mu_lgZSD = max(-0.21, -14.8*(obj.d_2D_out/1000)+0.01*abs(obj.UE.h_UT-obj.sector.h_BS)+0.83);
                        ...
                        obj.mu_lgZSD = max(-0.5, -3.1*(obj.d_2D_out/1000)+0.01*max(obj.UE.h_UT-obj.sector.h_BS, 0)+0.2);
            ...
            elseif  strcmp(obj.scenario.name,'InF')
                ...
                if obj.bLOS
                    ...
                    temp1 = obj.sector.corr_sos{1}.randn([obj.UE.pos,obj.UE.h_UT]');
                    ksi = reshape(temp1,[numel(temp1), 1]);
                    ksi = tab.InF_C_sqrt_LOS*ksi;
                    obj.SF = obj.sigma_SF*ksi(1);
                    obj.K = 8*ksi(2)+7;
%                     obj.DS = 10^(0.15*ksi(3)+ (log10(26*(obj.scenario.V/obj.scenario.S)+14)-9.35));
                    %%%%%%%%
                    obj.DS = 10^(0.18*ksi(2)+ (log10(0.76)+log10(70*(obj.scenario.V/obj.scenario.S)+10)-9-2.33*0.18));
                    %%%%%%%%
                    ...
                    temp1 = obj.sector.corr_sos{2}.randn([obj.UE.pos,obj.UE.h_UT]');
                    ksi = reshape(temp1,[numel(temp1), 1]);
                    ksi = tab.InF_C_sqrt_NLOS*ksi;
                    obj.SF = obj.sigma_SF*ksi(1);
%                     obj.DS = 10^(0.19*ksi(2)+ (log10(30*(obj.scenario.V/obj.scenario.S)+32)-9.44));
                    %%%%%%%%
                    obj.DS = 10^(0.18*ksi(2)+ (log10(70*(obj.scenario.V/obj.scenario.S)+10)-9-2.33*0.18));
                    %%%%%%%%
...
%             end
            ...
            C_phi_NLOS_TAB = [0.779, 0.860, 1.018, 1.090, 1.123, 1.146, 1.190, 1.211, 1.226, 1.273, 1.289, 1.289];% , 1.358
...
%             phi_n_AOD_tmp  = -obj.ASD*log(obj.Pn_LOS/max(obj.Pn_LOS))/C_phi;
            ...
            if ~isempty(obj.sector.SpatialConsistency)
                Xn = round(obj.sector.SpatialConsistency.AODsign{obj.spatialConsist_indx}.rand(obj.UE.initPos))*2-1;
                Yn = (obj.ASD/7)*obj.sector.SpatialConsistency.AODoffset{obj.spatialConsist_indx}.randn([obj.UE.pos,obj.UE.h_UT]');
                Xn = Xn(obj.tau_order);
                Yn = Yn(obj.tau_order);
                Xn = Xn(obj.keep).';
                Yn = Yn(obj.keep).';
...
%             C_phi_NLOS = C_phi_NLOS_TAB(obj.N);
            ...
            C_theta_NLOS_TAB = [0.889, 0.957, 1.031, 1.104, 1.1088, 1.184, 1.178, 1.178]; % 1.282
            ...
            if ~isempty(obj.sector.SpatialConsistency)
                Xn = round(obj.sector.SpatialConsistency.ZOAsign{obj.spatialConsist_indx}.rand(obj.UE.initPos))*2-1;
...
%             C_phi_NLOS = C_phi_NLOS_TAB(obj.N);
            ...
            if ~isempty(obj.sector.SpatialConsistency)
                ...
                Yn = (obj.ZSD/7)*obj.sector.SpatialConsistency.ZODoffset{obj.spatialConsist_indx}.randn([obj.UE.pos,obj.UE.h_UT]');
                Xn = Xn(obj.tau_order);
                Yn = Yn(obj.tau_order);
                Xn = Xn(obj.keep).';
                Yn = Yn(obj.keep).';
...
%             as  = min(as1,as2);
       ...
       function [ as, mean_angle ]  = calc_angular_spreads_(obj,ang,pow,LOSang,wrap_angles )
            % CALC_ANGULAR_SPREADS Calculates the angular spread in degree
            ...
            as = sqrt(-2*log(abs(sum( pow.*exp( 1j*ang ),2))));
%             as = sqrt( sum(pow.*(phi.^2),2)); %  - sum( pow.*phi,2).^2 
            mean_angle = mean_angle*180/pi;
            as = as*180/pi;
       end
...
%             ds = sqrt( sum(pow.*((tmp).^2)) ); %  - sum(pow.*tmp).^2
       ...
       function [ ds, mean_delay ] = calc_delay_spread_(obj, taus, pow )
       % CALC_DELAY_SPREAD Calculates the delay spread in [s]
